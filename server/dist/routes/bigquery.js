...
// Өгөгдөл татах
router.get('/wildfire', async (req, res) => {
    try {
        const limit = parseInt(req.query.limit) || 1000;
        const confidence = req.query.confidence || 'high';
        const query = `
...
// ML загварын үнэлгээ
router.get('/ml-results', async (req, res) => {
    try {
        const query = `
      SELECT *
      FROM ML.EVALUATE(
...
router.get('/landsat-stats', async (req, res) => {
    try {
        const query = `
      SELECT 
...
// Landsat ML үнэлгээ
router.get('/landsat-ml', async (req, res) => {
    try {
        const query = `
      SELECT *
      FROM ML.EVALUATE(
...
router.get('/landsat-by-satellite', async (req, res) => {
    try {
        const query = `
