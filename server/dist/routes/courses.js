...
router.get('/my-progress', async (req, res) => {
    try {
        ...
        const coursesResult = await pool.query(`SELECT * FROM courses WHERE published = true ORDER BY created_at ASC`);
        ...
        const courses = await Promise.all(coursesResult.rows.map(async (course) => {
            let progressRow = {};
            if (studentId) {
                const pr = await pool.query(`SELECT * FROM course_progress WHERE student_id = $1 AND course_id = $2`, [studentId, course.id]);
                progressRow = pr.rows[0] ?? {};
            }
            let progress = 0;
            if (progressRow.material_viewed)
                progress = 25;
            if (progressRow.exercise_submitted)
                progress = 50;
            if (progressRow.quiz_score != null)
                progress = 75;
            if (progressRow.certificate_issued)
                progress = 100;
            return {
                ...course,
...
router.patch('/:id/progress', async (req, res) => {
    try {
        ...
        const { student_id, material_viewed, exercise_submitted, exercise_accuracy, quiz_score, quiz_answers, certificate_issued, } = req.body;
        if (!student_id)
...
router.post('/', async (req, res) => {
    try {
        ...
        const result = await pool.query(`INSERT INTO courses (title, description, materials, parts, assignment, quiz, published)
