...
router.get('/students', async (_req, res) => {
    try {
        const studentsResult = await pool.query(`SELECT u.id, u.name, u.email
       FROM users u
       WHERE u.role = 'student'
       ORDER BY u.name`);
        const students = await Promise.all(studentsResult.rows.map(async (u) => {
            // Progress
            const progResult = await pool.query(`SELECT COUNT(*) AS completed FROM student_progress WHERE student_id = $1 AND completed = true`, [u.id]);
            const completed = Number(progResult.rows[0]?.completed ?? 0);
            const progress = Math.min(100, Math.round((completed / 3) * 100));
            // Latest submission
            const subResult = await pool.query(`SELECT s.id, s.accuracy, s.result_output as notes, s.submitted_at,
                  f.grade, f.comment as feedback
           FROM submissions s
           LEFT JOIN feedback f ON f.student_id = s.student_id
           WHERE s.student_id = $1
           ORDER BY s.submitted_at DESC`, [u.id]);
            ...
            const status = submissions.length > 0 ? 'submitted'
...
router.post('/approve', async (req, res) => {
    try {
        ...
        await pool.query(`INSERT INTO feedback (instructor_id, student_id, grade, comment)
       SELECT $1, student_id, 100, 'Батлагдлаа'
       FROM submissions WHERE id = $2
       ON CONFLICT DO NOTHING`, [instructor_id, submission_id]);
...
router.post('/quiz', async (req, res) => {
    try {
       ...
       SET quiz = $1
       WHERE id = $2`, [JSON.stringify({ questions, pass_threshold, time_limit }), course_id]);
        res.json({ success: true });
    }
    catch (err) {
...
router.get('/quiz/:courseId', async (req, res) => {
    try {
        const result = await pool.query('SELECT quiz FROM courses WHERE id = $1', [req.params.courseId]);
...
router.post('/quiz/submit', async (req, res) => {
    try {
        ...
        const result = await pool.query(`INSERT INTO quiz_submissions (student_id, course_id, answers, score, passed)
       VALUES ($1, $2, $3, $4, $5)
       RETURNING *`, [student_id, course_id, JSON.stringify(answers), score, passed]);
