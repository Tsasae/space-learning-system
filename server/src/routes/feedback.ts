...
router.post('/', async (req: Request, res: Response) => {
  try {
    ...
    const result = await pool.query(
      `INSERT INTO feedback (instructor_id, student_id, grade, comment, material_id)
       VALUES ($1, $2, $3, $4, $5) RETURNING *`,
      [instructor_id, student_id, grade, comment, material_id || null]
    );
...
router.get('/:student_id', async (req: Request, res: Response) => {
  try {
    const result = await pool.query(
      `SELECT f.*, u.name as instructor_name
       FROM feedback f
